# IntelliClave — FL + DP Integration

## 1. Overview

The project built a complete Federated Learning pipeline using Flower and FedAvg, then integrated Differential Privacy into that pipeline.

The integration follows a clean separation of concerns:

- FL pipeline: data loading, model architecture, FL server, FL client base, FedAvg aggregation
- DP layer: Opacus PrivacyEngine attachment, DP-SGD training, epsilon tracking, privacy budget monitoring, epsilon sweep experiments, all evaluation graphs
...
Local training runs with DP-SGD:
    ...
    ↓
Epsilon consumed is measured and added to metrics
    ↓
...
| `data_loader.py` | `privacy/` | ⚠️ Retired from FL pipeline. `data_utils.py` handles all data loading. Kept for reference only. |
| `dp_flower_client.py` | `privacy/` | ⚠️ Retired. DP is now integrated directly into `fl_client.py`. Kept for reference only. |
...
| `validate_model.py` | `privacy/` | Runs Opacus compatibility check on the model. Run this before any DP training if the model architecture changes. |
| `run_budget_monitor.py` | project root | Script to run budget monitor and print formatted summary table. Run after FL completes. |

### FL files extended with DP

| File | Location | DP Extensions Added |
|------|----------|---------------------|
...
### Why `train_one_epoch()` needs no changes

Opacus replaces the optimizer and DataLoader transparently during `make_private_with_epsilon()`. The same training loop continues to work — it calls `optimizer.step()` as normal, but the optimizer is now a `DPOptimizer` internally performing gradient clipping and noise addition before each update.
...
results/
├── fl_rounds/
│   ├── fl_metrics.json          — FL accuracy and loss per round
│   ├── fl_privacy.json          — Epsilon per client per round
...
```
[DP] Attaching PrivacyEngine: ε=10.0, δ=4.03e-04, total_epochs=5
[DP] PrivacyEngine attached successfully
  Epoch   1 loss=x.xxxx accuracy=x.xxxx macro_f1=x.xxxx ε=x.xxxx
  Epoch   5 loss=x.xxxx accuracy=x.xxxx macro_f1=x.xxxx ε=9.xxxx
[DP] Final privacy budget consumed: ε=9.xxxx
```
...
### Step 3 — Run FL baseline (no DP, for comparison)
...
Wait for:
```
Flower server running (5 rounds)...
```
...
Expected final result (baseline): accuracy ≈ 96–97%, macro-F1 ≈ 97%.
...
```
[Client 1][DP] PrivacyEngine attached: ε=10.0, δ=4.03e-04, total_epochs=15 (3 local × 5 rounds)
[Client 1][DP] loss=x.xxxx acc=x.xxxx ε=x.xxxx
```

**Expected server output per round:**
```
[Server][DP] Round 1 avg_ε=x.xxxx — saved to fl_privacy.json
...
The DP-trained model will be lower than the baseline — this gap is the **privacy cost** and is the key evaluation result.
...
| DP costs accuracy | Final accuracy < baseline (96.99%) | `global_model_eval.json` |
...
| `batch_size` | 32 | Set by `data_utils.py` |
