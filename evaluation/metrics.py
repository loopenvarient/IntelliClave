...
Class names and number of classes are passed at call time, so this module
works with any dataset — not just HAR.
"""
import json
...
def evaluate(
    ...
    Parameters
    ----------
    y_true      : array-like (N,)    ground truth labels (0-based)
    y_pred      : array-like (N,)    predicted labels (0-based)
    y_prob      : array-like (N, C)  predicted probabilities
    epsilon     : float              privacy budget spent so far
    round_num   : int                FL round number
    class_names : list of str        optional class name labels;
...
def print_summary(results: Dict) -> None:
    ...
    print(f"\n{'='*50}")
    print(f"Round {results['round']}  |  epsilon={results['epsilon']}")
    print(f"{'='*50}")
...
if __name__ == "__main__":
    ...
    y_prob_perfect = np.eye(N_CLASSES)[y_pred_perfect]
    r1 = evaluate(y_true, y_pred_perfect, y_prob_perfect,
                  epsilon=0.5, round_num=1, class_names=names)
    ...
    y_pred_random = np.random.randint(0, N_CLASSES, N)
    y_prob_random = np.random.dirichlet(np.ones(N_CLASSES), N)
    r2 = evaluate(y_true, y_pred_random, y_prob_random,
                  epsilon=1.0, round_num=2, class_names=names)
    ...
    y_prob_same = np.zeros((N, N_CLASSES))
    y_prob_same[:, 0] = 1.0
    r3 = evaluate(y_true, y_pred_same, y_prob_same,
                  epsilon=1.5, round_num=3, class_names=names)
